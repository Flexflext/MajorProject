...
public class ConstrainedChainIK : MonoBehaviour
{
    ...
    [SerializeField] private List<Vector3> constraines = new List<Vector3>();
    ...
    {
        if (constraines.Count != chainLength)
        {
            if (constraines.Count < chainLength)
            {
                constraines.Add(new Vector3(180, 180, 180));
            }
            else
            {
                constraines.RemoveAt(constraines.Count - 1);
            }
        }
    }
    ...
    private void FabricAlgortihm()
    {
        ...
        else
        {
            ...
            //for (int i = 0; i < constraines.Count; i++)
            //{
